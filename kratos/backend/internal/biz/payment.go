...
func (pc *PaymentUsecase) CreateOrder(ctx context.Context, uid uint64, plan, lang string) (CreatedOrder, error) {
	...
	amountCents := planAmounts(cfg)[plan] // 金额强制唯一，见 planAmounts
	currency := "USD"
...
func (pc *PaymentUsecase) Webhook(ctx context.Context, providerName string, rawBody []byte, headers map[string]string) error {
	...
	// plan 由金额反查（下单时 amount↔plan 一一对应）
	row, err := pc.providerRow(ctx, o.Provider)
	if err != nil {
		return pkg.ErrProviderOn
	}
	cfg, err := pc.decryptConfig(row.Config)
	...
	plan, days, ok := planForPrice(cfg, ev.AmountCents)
	if !ok {
		return pkg.ErrAmountMism
	}
...
func (pc *PaymentUsecase) providerRow(ctx context.Context, code string) (data.PaymentProvider, error) {
	var row data.PaymentProvider
	// FORCE_MASTER
	if err := pc.db.Clauses(gormdb.Write).WithContext(ctx).
		Where("code = ?", code).First(&row).Error; err != nil {
		return row, err
	}
	return row, nil
}
...
// planOrder 固定顺序：planForPrice 反查与金额唯一性校验都按此遍历，保证确定性。
var planOrder = []string{"monthly", "quarterly", "yearly"}

// planAmounts 返回三套餐生效金额；配置金额冲突（同价两套餐）时回退内置默认，
// 保证 amount↔plan 一一对应（幂等复用键与 webhook 反查的前提）。
func planAmounts(cfg map[string]any) map[string]int64 {
	out := map[string]int64{}
	seen := map[int64]string{}
	for _, p := range planOrder {
		a := planAmount(cfg, p)
		if prev, dup := seen[a]; dup {
			// ponytail: 冲突属配置错误，回退默认值；后续可改为校验时报错
			_ = prev
			for _, q := range planOrder {
				out[q] = defaultPlans[q]
			}
			return out
		}
		seen[a] = p
...
// planAmount 单套餐金额：provider 配置覆盖优先，否则内置默认。
func planAmount(cfg map[string]any, plan string) int64 {
	if plans, ok := cfg["plans"].(map[string]any); ok {
		if v, ok := plans[plan].(float64); ok && v > 0 {
			return int64(math.Round(v))
		}
	}
	return defaultPlans[plan]
}

// planForPrice 金额反查套餐：按固定顺序遍历（与下单同一映射，必中且唯一）。
func planForPrice(cfg map[string]any, cents int64) (string, int, bool) {
	amounts := planAmounts(cfg)
	for _, p := range planOrder {
		if amounts[p] == cents {
			return p, planDays[p], true
		}
	}
	return "", 0, false
}
