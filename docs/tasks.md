...
| 管理端 | T-A-17 | token 持久化（对齐 C4） | 收尾 | ⬜ 未开始 |
| 客户端 | T-C-01~04 | 多语言补齐（后端 lang 对齐 + 两端资源） | C1 | ✅ 已完成 |
| 客户端 | T-C-05~12 | 阅读体验（字号 / 主题 / 翻页 / 缓存 / 进度） | C2 | ✅ 已完成 |
| 客户端 | T-C-13~15 | 社区与发现（评论增强 / 分类 / 热搜） | C3 | ⬜ 未开始 |
| 客户端 | T-C-16~21 | 多端统一（分页 / token / 桌面布局） | C4 | ⬜ 未开始 |
| 客户端 | T-C-22~23 | 加载空态统一 + 全链路回归 | 收尾 | ⬜ 未开始 |
...
| 支付 | T-P-19~20 | 本地支付补强 / PayPal（二期） | 新增需求 | ⬜ 未开始 |
...
| 第三批 | T-A-12~13（M3）‖ T-C-13~15（C3） | C3 的举报入口依赖 T-A-06 已就绪 | ⏳ 进行中（T-A 完成，T-C-13~15 未开始） |
| 第四批 | T-A-14~16（M4/M5）‖ T-C-16~21（C4） | 均无强依赖 | ⏳ 进行中（T-A 完成，T-C-16~21 未开始） |
| 收尾 | T-A-17、T-C-22~23 | 统一收尾 + 回归 | ⬜ 未开始 |
...
**调度**：第一批 T-P-01~08（后端支付底座）→ T-P-09~13（管理端）→ T-P-14~17（客户端），2026-08-29 前已全部提交（✅）；T-P-18 随各批内嵌完成；T-P-19/20 二期按商户资质逐语言启用（⬜）。
