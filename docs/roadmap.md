...
| VIP / 支付 | `novel_vip_order` / `novel_payment_order` 表 + 支付服务（对接三方），VIP 章节校验链路 | 🟡 |
...
5. **VIP 缺失**：`IsVip` 字段与 api.md 已有表述，付费链路未打通，注意文档一致性。
