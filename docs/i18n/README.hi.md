...
<div align="center"><img src="../mascot.svg" alt="Open Novel का शुभंकर Novi" width="150"/></div>
...
- **खोज और खोजें**: बहुभाषी टोकनाइज़ेशन खोज, हॉट कीवर्ड रैंकिंग, खोज सुझाव (क्लाइंट-साइड लोकल हिस्ट्री 20 प्रविष्टियाँ + 200ms डिबाउंस सुझाव), AI अनुशंसाएँ, श्रेणी ब्राउज़िंग
- **एडमिन पैनल**: सामग्री मॉडरेशन, उपयोगकर्ता प्रबंधन, डेटा आँकड़े (डैशबोर्ड / DAU / रैंकिंग / व्यवहार विश्लेषण /api/stats/behavior), कॉन्फ़िगरेशन प्रबंधन (श्रेणी टैग), मशीन अनुवाद वर्कफ़्लो (DeepL, /api/admin/translate/*, एडमिन का «अनुवाद» पेज + मैनुअल एडिटिंग), ऑडिट लॉग क्वेरी (/api/admin/audit-logs), CDN विक्रेता प्रबंधन (बहु-विक्रेता कॉन्फ़िगरेशन / सक्षम-अक्षम / क्रम, हॉट-रिलोड तत्काल प्रभावी)
- **भुगतान और VIP**: 11 प्रदाताओं (Stripe, NOWPayments (USDT), Razorpay, KOMOJU, PortOne, Mercado Pago, Xendit, PayPal, Alipay, WeChat Pay Global, UnionPay) के माध्यम से मल्टी-चैनल भुगतान, VIP प्लान सदस्यता और नवीनीकरण, भाषा-आधारित भुगतान विधि रूटिंग (WeChat Pay Global एकीकृत, घरेलू WeChat Pay एकीकृत नहीं, चीनी मर्चेंट योग्यता आवश्यक)

## सिस्टम आर्किटेक्चर

<p align="center"><img src="images/hi/architecture.svg" alt="सिस्टम आर्किटेक्चर आरेख" width="860"/></p>

पूरा सिस्टम Go-Kratos माइक्रोसर्विस आर्किटेक्चर पर आधारित है: Flutter / HarmonyOS क्लाइंट Nginx + बहु-विक्रेता CDN (वैश्विक लाइन के लिए Cloudflare / CloudFront, चीन लाइन के लिए Aliyun / Tencent Cloud; एडमिन से कॉन्फ़िगर करने योग्य, कॉन्फ़िगरेशन फ़िंगरप्रिंट का हॉट-रिलोड तत्काल प्रभावी) के माध्यम से API गेटवे के साथ इंटरैक्ट करते हैं; गेटवे डोमेन के अनुसार उपयोगकर्ता, पुस्तकें, अध्याय, टिप्पणियाँ, खोज, अनुशंसा आदि बैकएंड सेवाओं तक रूट करता है; डेटा परत MySQL मास्टर-स्लेव (रीड-राइट सेपरेशन) + Redis कैश + OpenSearch सर्च इंडेक्स है। सेवाओं के बीच gRPC संचार होता है, बाहरी HTTP इंटरफेस का एकीकृत उपसर्ग `/api` है।
...
| गेटवे | Nginx + बहु-विक्रेता CDN (Cloudflare / CloudFront / Aliyun / Tencent Cloud), Go-Kratos API गेटवे (gRPC / HTTP दोहरा प्रोटोकॉल) |
| सर्वर | Go 1.22+, Kratos v2, protobuf / gRPC |
| स्टोरेज | MySQL 8.0 (मास्टर-स्लेव), Redis 7.x (Cluster), OpenSearch 2.x, Redis के ऊपर ristretto इन-प्रोसेस L1 कैश (30 सेकंड TTL) |
...
सभी कार्य श्रृंखलाएँ पूरी हो चुकी हैं।
