...
<div align="center"><img src="../mascot.svg" alt="Open Novel-এর মাসকট Novi" width="150"/></div>
...
- **সার্চ ও আবিষ্কার**: বহুভাষিক টোকেনাইজেশন সার্চ, হট কীওয়ার্ড র্যাঙ্কিং, সার্চ সাজেশন (ক্লায়েন্ট-সাইড লোকাল হিস্ট্রি ২০টি + ২০০ms ডিবাউন্স সাজেশন), AI রিকমেন্ডেশন, ক্যাটাগরি ব্রাউজিং
- **অ্যাডমিন প্যানেল**: কন্টেন্ট মডারেশন, ইউজার ম্যানেজমেন্ট, ডেটা পরিসংখ্যান (ড্যাশবোর্ড / DAU / র্যাঙ্কিং / আচরণ বিশ্লেষণ /api/stats/behavior), কনফিগারেশন ম্যানেজমেন্ট (ক্যাটাগরি ট্যাগ), মেশিন ট্রান্সলেশন ওয়ার্কফ্লো (DeepL, /api/admin/translate/*, অ্যাডমিনের «অনুবাদ» পেজ + ম্যানুয়াল এডিটিং), অডিট লগ কোয়েরি (/api/admin/audit-logs), CDN ভেন্ডর ম্যানেজমেন্ট (মাল্টি-ভেন্ডর কনফিগারেশন / সক্রিয়-নিষ্ক্রিয় / সাজানো, হট-রিলোড তাৎক্ষণিক কার্যকর)
- **পেমেন্ট ও VIP**: ১০টি প্রোভাইডারের (Stripe, NOWPayments (USDT), Razorpay, KOMOJU, PortOne, Mercado Pago, Xendit, PayPal, Alipay, WeChat Pay Global, UnionPay) মাধ্যমে মাল্টি-চ্যানেল পেমেন্ট, VIP প্ল্যান সাবস্ক্রিপশন ও নবায়ন, ভাষা-ভিত্তিক পেমেন্ট পদ্ধতি রাউটিং (WeChat Pay Global সংযুক্ত, দেশীয় WeChat Pay সংযুক্ত নয়, চীনা মার্চেন্ট যোগ্যতা প্রয়োজন)

## সিস্টেম আর্কিটেকচার

<p align="center"><img src="images/bn/architecture.svg" alt="সিস্টেম আর্কিটেকচার ডায়াগ্রাম" width="860"/></p>

পুরো সিস্টেমটি Go-Kratos মাইক্রোসার্ভিস আর্কিটেকচারের উপর ভিত্তি করে তৈরি: Flutter / HarmonyOS ক্লায়েন্ট Nginx + মাল্টি-ভেন্ডর CDN (গ্লোবাল লাইনের জন্য Cloudflare / CloudFront, চায়না লাইনের জন্য Aliyun / Tencent Cloud; অ্যাডমিন থেকে কনফিগার করা যায়, কনফিগারেশন ফিঙ্গারপ্রিন্টের হট-রিলোড তাৎক্ষণিক কার্যকর) এর মাধ্যমে API গেটওয়ের সাথে ইন্টারঅ্যাক্ট করে; গেটওয়ে ডোমেইন অনুযায়ী ব্যবহারকারী, বই, অধ্যায়, মন্তব্য, সার্চ, রিকমেন্ডেশন ইত্যাদি ব্যাকএন্ড সার্ভিসে রাউট করে; ডেটা লেয়ার হলো MySQL মাস্টার-স্লেভ (রিড-রাইট সেপারেশন) + Redis ক্যাশ + OpenSearch সার্চ ইনডেক্স। সার্ভিসগুলোর মধ্যে gRPC কমিউনিকেশন হয়, বাহ্যিক HTTP ইন্টারফেসের ইউনিফাইড প্রিফিক্স `/api`।
...
| গেটওয়ে | Nginx + মাল্টি-ভেন্ডর CDN (Cloudflare / CloudFront / Aliyun / Tencent Cloud), Go-Kratos API গেটওয়ে (gRPC / HTTP দ্বৈত প্রোটোকল) |
| সার্ভার | Go 1.22+, Kratos v2, protobuf / gRPC |
| স্টোরেজ | MySQL 8.0 (মাস্টার-স্লেভ), Redis 7.x (Cluster), OpenSearch 2.x, Redis-এর উপরে ristretto ইন-প্রসেস L1 ক্যাশ (৩০ সেকেন্ড TTL) |
