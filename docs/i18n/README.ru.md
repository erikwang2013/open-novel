...
- **Админ-панель**: модерация контента, управление пользователями, статистика, управление конфигурацией, страница запросов журнала аудита (пагинация + фильтрация по нескольким условиям)
- **Платежи и VIP**: мультиканальные платежи через 9 провайдеров (Stripe, NOWPayments (USDT), Razorpay, KOMOJU, PortOne, Mercado Pago, Xendit, PayPal, Alipay), подписка и продление VIP-планов, маршрутизация способов оплаты по языку (WeChat Pay не подключён, требуется статус мерчанта)
